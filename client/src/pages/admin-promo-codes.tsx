    ...
    <AppShell contentClassName="content-area">
      ...
      <div className="form-card" style={{ margin: "0 24px 20px" }}>
        <div style={{ marginBottom: 16 }}>
          <h2 style={{ fontFamily: "var(--fh)", fontSize: 16, fontWeight: 600, color: "var(--card-heading)", margin: 0 }}>
            Create New Code
          </h2>
        </div>
        <form
      ...
      <div className="form-card" style={{ margin: "0 24px 20px" }}>
        ...
        {codes.length === 0 ? (
          <p style={{ fontSize: 13, color: "var(--card-body)" }}>No promo codes yet.</p>
        ) : (
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
              <thead>
                <tr style={{ borderBottom: "1px solid var(--card-border-subtle)", textAlign: "left" }}>
                  <th style={{ padding: "8px 12px", fontWeight: 600, color: "var(--card-label)", fontSize: 10, textTransform: "uppercase", letterSpacing: "0.5px" }}>Code</th>
                  <th style={{ padding: "8px 12px", fontWeight: 600, color: "var(--card-label)", fontSize: 10, textTransform: "uppercase", letterSpacing: "0.5px" }}>Tokens</th>
                  <th style={{ padding: "8px 12px", fontWeight: 600, color: "var(--card-label)", fontSize: 10, textTransform: "uppercase", letterSpacing: "0.5px" }}>LC</th>
                  <th style={{ padding: "8px 12px", fontWeight: 600, color: "var(--card-label)", fontSize: 10, textTransform: "uppercase", letterSpacing: "0.5px" }}>Used / Max</th>
              ...
              <tbody>
                {codes.map((c) => (
                  <tr key={c.id} style={{ borderBottom: "1px solid var(--card-border-subtle)" }}>
                    ...
                    <td style={{ padding: "10px 12px" }}>
                      <span style={{
                        ...
                        fontSize: 11,
                        fontWeight: 600,
                        background: c.currentRedemptions >= c.maxRedemptions ? "rgba(239,68,68,0.15)" : c.isActive ? "rgba(74,140,111,0.15)" : "rgba(234,179,8,0.15)",
                        color: c.currentRedemptions >= c.maxRedemptions ? "#f87171" : c.isActive ? "#4ade80" : "#fbbf24",
                      }}>
