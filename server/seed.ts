  ...
  const rules: Array<{
    ...
    // ── 48. CA ACI eManifest (dest: CA) ──
    {
      ...
      validationSpec: {
        ...
        expectedFields: [
          // Conveyance / Trip level (ECCRD Chapter 7)
          { name: "carrierCode", description: "CBSA-assigned 4-character carrier code (SCAC or IATA)", required: true, severityIfMissing: "critical" },
          { name: "conveyanceReference", description: "Conveyance Reference Number (CRN) / trip number — unique per trip", required: true, severityIfMissing: "critical" },
          { name: "portOfEntry", description: "3-digit CBSA port of entry code", required: true, severityIfMissing: "critical" },
          ...
          { name: "modeOfTransport", description: "Mode of transport (sea, air, highway, rail)", required: true, severityIfMissing: "warning" },
          { name: "vesselOrVehicleId", description: "Vessel name / truck number / flight number / railcar ID", required: true, severityIfMissing: "warning" },
          { name: "sealNumbers", description: "Seal number(s) on trailers/containers", required: true, severityIfMissing: "warning" },
          // Cargo / Shipment level (ECCRD — carrier or freight forwarder)
          { name: "cargoControlNumber", description: "Cargo Control Number (CCN) — unique shipment identifier, exactly as on cargo control document including hyphens", required: true, severityIfMissing: "critical" },
          { name: "shipmentType", description: "Shipment type (e.g., PARS, In-bond, Sufferance)", required: true, severityIfMissing: "warning" },
          { name: "shipperName", description: "Shipper (ship-from) name, address, city, province/state, country, postal code", required: true, severityIfMissing: "critical" },
          { name: "consigneeName", description: "Consignee (deliver-to) name, address, city, province/state, country, postal code", required: true, severityIfMissing: "critical" },
          { name: "notifyParty", description: "Notify party name and address (if different from consignee)", required: false, severityIfMissing: "info" },
          { name: "goodsDescription", description: "Commodity description of goods", required: true, severityIfMissing: "critical" },
          { name: "numberOfPieces", description: "Number of pieces / packages", required: true, severityIfMissing: "critical" },
          { name: "grossWeight", description: "Gross weight and unit of measure", required: true, severityIfMissing: "critical" },
          { name: "containerNumbers", description: "Container number(s) (ISO 6346 format)", required: true, severityIfMissing: "critical" },
          { name: "undgCode", description: "UNDG dangerous goods UN number and hazmat indicator (if applicable)", required: false, severityIfMissing: "warning" },
          // House Bill level (ECCRD Chapter 8 — freight forwarder)
          { name: "houseBillNumber", description: "House bill number (if freight forwarder consolidation)", required: false, severityIfMissing: "info" },
          { name: "masterBillReference", description: "Master bill / CCN reference linking house bill to conveyance", required: false, severityIfMissing: "info" },
          // Driver info (highway mode — ECCRD Chapter 7)
          { name: "driverInfo", description: "Driver name, date of birth, citizenship, travel document type/number (highway mode)", required: false, severityIfMissing: "info" },
        ],
        consistencyChecks: [
          { type: "hs_prefix_matches_context", severity: "warning", message: "HS code does not match expected commodity." },
          { type: "numeric_positive", severity: "critical", message: "Gross weight must be positive." },
          { type: "container_number_format", severity: "warning", message: "Container number does not match ISO 6346 format." },
          { type: "destination_country_match", severity: "warning", message: "Port of entry should be a Canadian port." },
        ],
        minimumAcceptable: { mustHave: ["cargoControlNumber", "consigneeName", "portOfEntry", "containerNumbers", "carrierCode"], ifMissing: "ISSUES_FOUND" },
        outputHints: {
          notes: "ACI eManifest is Canada's equivalent of the US ISF 10+2. Data organized into Conveyance (trip) → Cargo (shipment) → House Bill levels per ECCRD. Filing deadlines: ocean = 24 hours before loading, highway = 1 hour pre-arrival, air = varies (4 hours short-haul, pre-departure long-haul). Non-compliance = CBSA do-not-load (DNL) directive. Carrier files conveyance/cargo data; freight forwarder files supplementary house bill data (ECCRD Chapter 8). Reference: ECCRD Chapter 7 (cbsa-asfc.gc.ca/prog/manif/eccrdhi-deccerout-eng.html).",
        },
      },
    },
    // ── 49. CA CFIA (dest: CA, trigger: sps) ──
    {
      ...
      validationSpec: {
        ...
        expectedFields: [
          // SFC Licence fields (My CFIA portal application)
          { name: "sfcLicenceNumber", description: "Safe Food for Canadians (SFC) licence number", required: true, severityIfMissing: "critical" },
          { name: "licenceActivityType", description: "SFC licence activity type (Import, Export, Interprovincial trade, Manufacturing, etc.)", required: true, severityIfMissing: "warning" },
          { name: "foodCommodityType", description: "Food commodity category (processed fruits/vegetables, fish, dairy, meat, eggs, honey, maple, fresh produce, organic)", required: true, severityIfMissing: "warning" },
          // Parties
          { name: "importerName", description: "Canadian importer legal business name", required: true, severityIfMissing: "critical" },
          { name: "importerBusinessNumber", description: "Importer's 15-digit Business Number (BN15)", required: true, severityIfMissing: "critical" },
          { name: "importerAddress", description: "Importer full address including province and postal code", required: true, severityIfMissing: "warning" },
          { name: "exporterName", description: "Foreign exporter/supplier name and address", required: true, severityIfMissing: "critical" },
          { name: "contactPerson", description: "Contact person name, email, phone for CFIA communication", required: true, severityIfMissing: "warning" },
          // Product / commodity
          { name: "productDescription", description: "Description of food/plant/animal product", required: true, severityIfMissing: "critical" },
          { name: "scientificName", description: "Scientific name (genus/species) — required for plant import permits (Form CFIA/ACIA 5256)", required: false, severityIfMissing: "warning" },
          ...
          { name: "countryOfOrigin", description: "Country of origin (separate permit per country for plant imports)", required: true, severityIfMissing: "critical" },
          { name: "hsCode", description: "Canadian tariff classification code (10-digit)", required: true, severityIfMissing: "critical" },
          { name: "quantity", description: "Quantity and unit of measure", required: true, severityIfMissing: "warning" },
          { name: "intendedEndUse", description: "Intended end use (propagation, consumption, processing, research, ornamental)", required: false, severityIfMissing: "info" },
          ...
          { name: "importPermitRef", description: "CFIA plant import permit number — Form CFIA/ACIA 5256, Section 32 (if required per AIRS)", required: false, severityIfMissing: "warning" },
          { name: "section43PermitRef", description: "Section 43 permit for otherwise prohibited material (research/education/exhibition only — requires containment facility details and disposal plan)", required: false, severityIfMissing: "warning" },
          { name: "veterinaryPermitRef", description: "Veterinary import permit reference (if animal product)", required: false, severityIfMissing: "warning" },
          { name: "healthCertificateRef", description: "Health certificate reference (if animal-origin product)", required: false, severityIfMissing: "warning" },
          // Compliance
          { name: "preventiveControlPlan", description: "Attestation that written Preventive Control Plan (PCP) exists per SFCR", required: true, severityIfMissing: "warning" },
          { name: "labelCompliance", description: "Confirmation labels comply with CFIA/SFCR requirements (bilingual English/French)", required: true, severityIfMissing: "warning" },
          ...
          // Destination
          { name: "portOfEntry", description: "Intended CBSA port of entry", required: false, severityIfMissing: "info" },
          { name: "canadianDestination", description: "Canadian destination address where goods will be received/stored", required: false, severityIfMissing: "info" },
        ],
        consistencyChecks: [
          { type: "origin_country_match", severity: "warning", message: "Country of origin does not match trade context." },
          { type: "hs_prefix_matches_context", severity: "warning", message: "HS code does not match expected commodity." },
        ],
        minimumAcceptable: { mustHave: ["sfcLicenceNumber", "importerBusinessNumber", "productDescription", "airsRequirements", "countryOfOrigin"], ifMissing: "ISSUES_FOUND" },
        outputHints: {
          notes: "SFC licence is mandatory for most food imports into Canada — apply via My CFIA portal. Use the AIRS database (airs-sari.inspection.gc.ca) to check commodity-specific requirements. Plant imports use Form CFIA/ACIA 5256 (Section 32 for permitted material, Section 43 for prohibited material). Section 43 permits only issued for scientific research, education, or exhibition — requires containment facility details. Scientific names (genus minimum) are mandatory on plant import permits — common names will be rejected. Preventive Control Plan attestation required under SFCR. Reference: CBSA Customs Notice 24-03 for SFC licence requirements.",
        },
      },
    },
    ...
    // ── 51. CA CBSA CARM (dest: CA) ──
    {
      ...
      validationSpec: {
        ...
        expectedFields: [
          // CAD Declaration Header (per D17-1-10 memorandum field coding)
          { name: "transactionNumber", description: "Field 1: CARM transaction number (replaces B3 entry number)", required: true, severityIfMissing: "critical" },
          { name: "declarationType", description: "Declaration type code (Type C = combined release+accounting, Type F = final accounting, Type B = accounting only)", required: true, severityIfMissing: "critical" },
          { name: "importerBusinessNumber", description: "Importer BN15 — 15-digit Business Number (9-digit BN + RM import/export account)", required: true, severityIfMissing: "critical" },
          { name: "customsBrokerBN", description: "Customs broker Business Number (if broker-filed)", required: false, severityIfMissing: "info" },
          { name: "cargoControlNumber", description: "Cargo Control Number (CCN) — exactly as on cargo control document, including hyphens", required: true, severityIfMissing: "critical" },
          { name: "releaseOffice", description: "Field 2: 3-digit CBSA office of release / port of entry", required: true, severityIfMissing: "warning" },
          { name: "modeOfTransport", description: "Mode of transport code", required: true, severityIfMissing: "warning" },
          { name: "dateOfRelease", description: "Date goods released by CBSA", required: true, severityIfMissing: "warning" },
          { name: "dateOfDirectShipment", description: "Date of direct shipment to Canada", required: true, severityIfMissing: "warning" },
          { name: "currencyCode", description: "Currency code for settlement", required: true, severityIfMissing: "warning" },
          { name: "exchangeRate", description: "Exchange rate to CAD", required: true, severityIfMissing: "warning" },
          // CAD Invoice / Vendor level
          { name: "vendorName", description: "Vendor/exporter name and address", required: true, severityIfMissing: "critical" },
          { name: "vendorInvoiceNumber", description: "Vendor's invoice number", required: true, severityIfMissing: "critical" },
          { name: "invoiceDate", description: "Invoice date", required: true, severityIfMissing: "warning" },
          { name: "purchaserName", description: "Purchaser name and address (if different from importer)", required: false, severityIfMissing: "info" },
          { name: "termsOfSale", description: "Terms and conditions of sale (e.g., CIF, FOB)", required: true, severityIfMissing: "warning" },
          // CAD Commodity Line level
          { name: "countryOfOrigin", description: "Field 25: Country of origin code", required: true, severityIfMissing: "critical" },
          { name: "countryOfExport", description: "Field 26: Country of export/direct shipment", required: true, severityIfMissing: "warning" },
          { name: "provinceOfDestination", description: "Field 27: Province of final destination", required: true, severityIfMissing: "warning" },
          { name: "quantityAndUOM", description: "Field 33: Quantity and unit of measure", required: true, severityIfMissing: "critical" },
          { name: "valueForDutyCode", description: "Field 37: Value for Duty (VFD) code — valuation method 1-6", required: true, severityIfMissing: "critical" },
          { name: "valueForDutyAmount", description: "Field 38: Value for duty amount (CAD)", required: true, severityIfMissing: "critical" },
          { name: "tariffTreatment", description: "Field 54: Tariff treatment code (MFN, GPT, LDCT, CPTPP, CETA, etc.)", required: true, severityIfMissing: "critical" },
          { name: "tariffClassification", description: "Field 58: HS tariff classification number (10-digit Canadian)", required: true, severityIfMissing: "critical" },
          { name: "chapter99TariffItem", description: "Field 59: Chapter 99 tariff item code (if applicable — surtax, anti-dumping)", required: false, severityIfMissing: "info" },
          // Duties & taxes
          { name: "customsDuty", description: "Customs duty amount", required: true, severityIfMissing: "critical" },
          { name: "exciseTax", description: "Excise tax / excise duty amount (if applicable)", required: false, severityIfMissing: "info" },
          { name: "gstAmount", description: "GST (5%) amount payable", required: true, severityIfMissing: "critical" },
          { name: "simaDuty", description: "SIMA anti-dumping/countervailing duty (if applicable)", required: false, severityIfMissing: "warning" },
          { name: "surtax", description: "Surtax amount (e.g., China surtax, if applicable)", required: false, severityIfMissing: "info" },
          { name: "totalDutiesAndTaxes", description: "Total duties + taxes + other levies", required: true, severityIfMissing: "critical" },
          // Origin proof for preferential claims
          { name: "proofOfOrigin", description: "Origin proof reference for preferential tariff claim (CPTPP: Annex 3-B cert; CETA: origin declaration; GPT/LDCT: Form A or exporter statement)", required: false, severityIfMissing: "warning" },
          { name: "proofOfOriginType", description: "Type of origin proof (CPTPP certificate, CETA origin declaration, Form A, exporter statement)", required: false, severityIfMissing: "info" },
        ],
        consistencyChecks: [
          { type: "origin_country_match", severity: "warning", message: "Country of origin does not match trade context." },
          ...
          { type: "date_not_future", severity: "warning", message: "Date of release should not be in the future." },
        ],
        minimumAcceptable: { mustHave: ["transactionNumber", "importerBusinessNumber", "cargoControlNumber", "tariffClassification", "valueForDutyAmount", "tariffTreatment"], ifMissing: "ISSUES_FOUND" },
        outputHints: {
          notes: "CARM Client Portal replaced ACROSS/B3 paper process in October 2024. CAD has three structural levels: Declaration Header → Invoice/Vendor → Commodity Line (per D17-1-10). Accounting data due within 5 business days of release. VFD codes: 1=transaction value, 2=identical goods, 3=similar goods, 4=deductive, 5=computed, 6=residual. GPT and LDCT available for qualifying African goods. CPTPP requires Annex 3-B certification (11 data elements including certifier type, origin criterion, blanket period). CETA uses exporter self-declaration (EU exporters need REX for >EUR 6,000). GPT/LDCT accepts Form A or exporter's statement of origin. Full field coding: CBSA Memorandum D17-1-10.",
        },
      },
    },
    // ── 51b. CA Canada Customs Invoice CI1 (dest: CA) ──
    {
      ruleKey: "ca_customs_invoice_ci1",
      destinationIso2: "CA",
      titleTemplate: "Canada Customs Invoice (Form CI1) or equivalent commercial invoice with required fields",
      descriptionTemplate: "CBSA may require a Canada Customs Invoice (Form CI1) or commercial invoice containing all 25 prescribed fields per Memorandum D1-4-1. The CI1 provides customs value, origin, and transaction detail needed for CAD filing. A commercial invoice is accepted if it contains all required information.",
      issuedByTemplate: "Exporter / Vendor",
      whenNeeded: "Before import — must accompany goods for customs accounting",
      tipTemplate: "If your commercial invoice already contains all CI1 fields (vendor, consignee, origin, transport conditions, value breakdowns), you can use it instead of a separate CI1. Check Field 23-25 for valuation adjustments (freight, insurance, royalties).",
      portalGuideTemplate: "CI1 Form — https://www.cbsa-asfc.gc.ca/publications/forms-formulaires/ci1-eng.html",
      documentCode: null,
      isSupplierSide: true,
      owner: "SUPPLIER",
      dueBy: "BEFORE_LOADING",
      ...
      validationSpec: {
        docTypeGate: {
          mustContainAny: ["Canada Customs Invoice", "CI1", "customs invoice", "CBSA invoice", "Canada invoice"],
          shouldContainAny: ["commercial invoice", "facture", "invoice"],
          rejectIfContainsAny: ["bill of lading", "packing list", "certificate of origin", "phytosanitary"],
        },
        expectedFields: [
          // CI1 Fields 1-5: Parties
          { name: "vendorName", description: "Field 1: Vendor name and address", required: true, severityIfMissing: "critical" },
          { name: "dateOfDirectShipment", description: "Field 2: Date of direct shipment to Canada", required: true, severityIfMissing: "critical" },
          { name: "otherReferences", description: "Field 3: Other references (PO number, job number, commercial invoice number)", required: false, severityIfMissing: "info" },
          { name: "consigneeName", description: "Field 4: Consignee name and address in Canada", required: true, severityIfMissing: "critical" },
          { name: "purchaserName", description: "Field 5: Purchaser name and address (if different from consignee)", required: false, severityIfMissing: "info" },
          // CI1 Fields 6-9: Origin and transport
          { name: "countryOfTransshipment", description: "Field 6: Country of transshipment", required: false, severityIfMissing: "info" },
          { name: "countryOfOrigin", description: "Field 7: Country of origin of goods", required: true, severityIfMissing: "critical" },
          { name: "transportConditions", description: "Field 8: Transportation conditions, mode of shipment, and place of direct shipment to Canada", required: true, severityIfMissing: "warning" },
          { name: "currencyOfSettlement", description: "Field 9: Currency of settlement", required: true, severityIfMissing: "critical" },
          // CI1 Fields 10-17: Goods and values
          { name: "numberOfPackages", description: "Field 10: Number and kind of packages", required: true, severityIfMissing: "warning" },
          { name: "commodityDescription", description: "Field 11: Specification of commodities — general then detailed description, condition of goods", required: true, severityIfMissing: "critical" },
          { name: "quantity", description: "Field 12: Quantity with unit of measure", required: true, severityIfMissing: "critical" },
          { name: "unitPrice", description: "Field 13: Unit price", required: true, severityIfMissing: "critical" },
          { name: "lineTotal", description: "Field 14: Total (extended price per line)", required: true, severityIfMissing: "critical" },
          { name: "totalWeight", description: "Field 16: Total weight (net / gross)", required: true, severityIfMissing: "warning" },
          { name: "invoiceTotal", description: "Field 17: Invoice total", required: true, severityIfMissing: "critical" },
          // CI1 Fields 19-20: Additional parties
          { name: "exporterName", description: "Field 19: Exporter name and address (if other than vendor)", required: false, severityIfMissing: "info" },
          { name: "originatorName", description: "Field 20: Originator — name and title of responsible person in vendor's organization with knowledge of the transaction", required: true, severityIfMissing: "warning" },
          // CI1 Fields 21-25: Valuation adjustments
          { name: "departmentalRuling", description: "Field 21: Departmental ruling number and date (if applicable)", required: false, severityIfMissing: "info" },
          { name: "transportChargesIncluded", description: "Field 23: Transportation charges from place of direct shipment — if INCLUDED in Field 17, indicate amount", required: false, severityIfMissing: "warning" },
          { name: "transportChargesNotIncluded", description: "Field 24: Transportation charges from place of direct shipment — if NOT included in Field 17, indicate amount", required: false, severityIfMissing: "warning" },
          { name: "otherCosts", description: "Field 25: Construction/erection/assembly/maintenance costs, royalty payments, export packing costs after importation", required: false, severityIfMissing: "info" },
        ],
        consistencyChecks: [
          { type: "origin_country_match", severity: "warning", message: "Country of origin does not match trade context." },
          { type: "numeric_positive", severity: "warning", message: "Prices and totals must be positive values." },
          { type: "currency_code_valid", severity: "warning", message: "Currency of settlement must be valid ISO 4217." },
          { type: "date_not_future", severity: "warning", message: "Date of direct shipment should not be in the future." },
        ],
        minimumAcceptable: { mustHave: ["vendorName", "consigneeName", "countryOfOrigin", "commodityDescription", "invoiceTotal"], ifMissing: "ISSUES_FOUND" },
        outputHints: {
          notes: "The CI1 has 25 numbered fields per CBSA Memorandum D1-4-1. Fields 23-25 are critical for customs valuation — they capture transportation charges and post-importation costs that affect value for duty. Field 18 checkbox: if any of fields 1-17 are on an attached commercial invoice, check this box. A commercial invoice can substitute for CI1 if it contains all required information. Reference: cbsa-asfc.gc.ca/publications/forms-formulaires/ci1-eng.html",
        },
      },
    },
    // ── 51c. CA FTA Origin Proof (dest: CA) ──
    {
      ruleKey: "ca_fta_origin_proof",
      destinationIso2: "CA",
      titleTemplate: "Canadian preferential tariff origin proof (CPTPP / CETA / GPT / LDCT certification)",
      descriptionTemplate: "To claim preferential tariff treatment (reduced or zero duty) under CPTPP, CETA, GPT, LDCT, or other Canadian FTAs, the importer must hold valid proof of origin. Format varies by agreement: CPTPP uses a certificate of origin (Annex 3-B), CETA uses an exporter origin declaration, GPT/LDCT accepts Form A or an exporter's statement.",
      issuedByTemplate: "Exporter, Producer, or Importer (self-certification varies by agreement)",
      whenNeeded: "At time of import — must be available for customs accounting and verification",
      tipTemplate: "For African goods, GPT (General Preferential Tariff) or LDCT (Least Developed Country Tariff) are the most relevant preferences. Form A or a simple exporter's statement of origin is accepted. Ensure the origin statement identifies the goods, states they originate in the beneficiary country, and references the invoice number(s).",
      portalGuideTemplate: "CBSA Memorandum D11-4-4 (GPT/LDCT) — https://www.cbsa-asfc.gc.ca/publications/dm-md/d11/d11-4-4-eng.html",
      documentCode: null,
      isSupplierSide: true,
      owner: "SUPPLIER",
      dueBy: "BEFORE_LOADING",
      ...
      validationSpec: {
        docTypeGate: {
          mustContainAny: ["certificate of origin", "origin declaration", "origin proof", "Form A", "GSP", "preferential origin", "CPTPP origin", "CETA origin", "GPT", "LDCT"],
          rejectIfContainsAny: ["invoice", "bill of lading", "packing list", "phytosanitary"],
        },
        expectedFields: [
          // Common fields across all Canadian FTA proofs
          { name: "certifierType", description: "Whether the certifier is the exporter, producer, or importer (CPTPP Annex 3-B element 1)", required: true, severityIfMissing: "critical" },
          { name: "certifierName", description: "Certifier full name, address (including country), telephone, email", required: true, severityIfMissing: "critical" },
          { name: "exporterName", description: "Exporter name, address (must be in a party country), telephone, email", required: true, severityIfMissing: "critical" },
          { name: "exporterReferenceNumber", description: "Exporter reference number — CETA: customs authorisation/REX number (EU) or Business Number (CA); GPT: not applicable", required: false, severityIfMissing: "warning" },
          { name: "producerName", description: "Producer name, address, telephone, email (or 'Various' / 'Available upon request' for multiple producers)", required: false, severityIfMissing: "warning" },
          { name: "importerName", description: "Importer name, address, email, telephone (if known; address in party country)", required: false, severityIfMissing: "info" },
          { name: "goodsDescription", description: "Description of the good(s) — sufficient to relate to the certification", required: true, severityIfMissing: "critical" },
          { name: "hsCode", description: "HS tariff classification (6-digit level)", required: true, severityIfMissing: "critical" },
          { name: "originCriterion", description: "Specific rule of origin under which the good qualifies (wholly obtained, sufficiently processed, cumulation, etc.)", required: true, severityIfMissing: "critical" },
          { name: "originDesignation", description: "Origin country or region designation (e.g., 'Canada', 'EU', beneficiary country name for GPT/LDCT)", required: true, severityIfMissing: "critical" },
          { name: "blanketPeriod", description: "Blanket period covering multiple shipments of identical goods (up to 12 months)", required: false, severityIfMissing: "info" },
          { name: "invoiceNumber", description: "Invoice number (if known, for single shipments)", required: false, severityIfMissing: "warning" },
          { name: "certificationStatement", description: "Signed certification statement attesting goods qualify as originating and information is true and accurate", required: true, severityIfMissing: "critical" },
          { name: "signatureAndDate", description: "Authorized signature and date of certification", required: true, severityIfMissing: "critical" },
          // GPT/LDCT specific (Form A fields)
          { name: "marksAndNumbers", description: "Marks and numbers of packages (Form A Field 6)", required: false, severityIfMissing: "info" },
          { name: "issuingAuthority", description: "Designated government authority certification and stamp (Form A Field 11 — GPT/LDCT only)", required: false, severityIfMissing: "warning" },
        ],
        consistencyChecks: [
          { type: "origin_country_match", severity: "critical", message: "Origin country does not match trade context or beneficiary country list." },
          { type: "hs_prefix_matches_context", severity: "warning", message: "HS code does not match expected commodity." },
        ],
        minimumAcceptable: { mustHave: ["exporterName", "goodsDescription", "originCriterion", "originDesignation", "certificationStatement"], ifMissing: "ISSUES_FOUND" },
        outputHints: {
          notes: "Canada accepts different origin proof formats per FTA: (1) CPTPP — Annex 3-B certificate with 11 data elements, self-certified by exporter/producer/importer; (2) CETA — exporter origin declaration on invoice/commercial doc, EU exporters need REX for shipments >EUR 6,000, Canadian exporters use Business Number; (3) GPT/LDCT — Form A (GSP standard form with 12 fields + government authority stamp) OR simpler exporter's statement of origin identifying goods, stating origin, and referencing invoice(s). For African goods, GPT/LDCT is most relevant — check CBSA Memorandum D11-4-4 for beneficiary country list. References: D11-4-14 (CETA), D11-4-2 (general proof of origin).",
        },
      },
    },
